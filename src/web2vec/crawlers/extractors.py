...
from web2vec.utils import get_domain_from_url, fetch_url, is_simple_type
...
class Extractor:
    ...
    def extract_features(self,
                         response: Response | ReqResponse | ReqResponse) -> FEATURE_CLASS:
...
class DNSExtractor(Extractor):
    ...
    def extract_features(self,
                         response: Response | ReqResponse) -> DNSFeatures:
...
class HtmlBodyExtractor(Extractor):
    ...
    def extract_features(self,
                         response: Response | ReqResponse) -> HtmlBodyFeatures:
...
class HttpResponseExtractor(Extractor):
    ...
    def extract_features(self,
                         response: Response | ReqResponse) -> HttpResponseFeatures:
        response.status_code = getattr(response, "status",
...
class CertificateExtractor(Extractor):
    ...
    def extract_features(self,
                         response: Response | ReqResponse) -> CertificateFeatures:
...
class UrlGeoExtractor(Extractor):
    ...
    def extract_features(self,
                         response: Response | ReqResponse) -> URLGeoFeatures:
...
class UrlLexicalExtractor(Extractor):
    ...
    def extract_features(self,
                         response: Response | ReqResponse) -> URLLexicalFeatures:
...
class WhoisExtractor(Extractor):
    ...
    def extract_features(self,
                         response: Response | ReqResponse) -> WhoisFeatures:
        return get_whois_features_cached(
            domain=get_domain_from_url(response.url))
...
class GoogleIndexExtractor(Extractor):
    ...
    def extract_features(self,
                         response: Response | ReqResponse) -> GoogleIndexFeatures:
...
class OpenPageRankExtractor(Extractor):
    ...
    def extract_features(self,
                         response: Response | ReqResponse) -> OpenPageRankFeatures:
...
class OpenPhishExtractor(Extractor):
    ...
    def extract_features(self,
                         response: Response | ReqResponse) -> OpenPhishFeatures:
...
class PhishTankExtractor(Extractor):
    ...
    def extract_features(self,
                         response: Response | ReqResponse) -> PhishTankFeatures:
        return get_phishtank_features_cached(
            domain=get_domain_from_url(response.url))
...
class SimilarWebExtractor(Extractor):
    ...
    def extract_features(self,
                         response: Response | ReqResponse) -> SimilarWebFeatures:
        return get_similar_web_features_cached(
            domain=get_domain_from_url(response.url))
...
class UrlHausExtractor(Extractor):
    ...
    def extract_features(self,
                         response: Response | ReqResponse) -> URLHausFeatures:
        return get_url_haus_features_cached(
            domain=get_domain_from_url(response.url))
...
def process_extractors(url: str, extractors: List[Extractor]) -> dict:
    extractors_result = {}
    try:
        ...
        for extractor in extractors:
            try:
                ...
                extractors_result.update(
                    {f"{extractor.FEATURE_TYPE}_{key}": value for key, value in
                     result_as_dict.items()})
