...
class Config(BaseSettings):
    ...
    open_page_rank_api_key: Optional[str] = Field(env='WEB2VEC_OPEN_PAGERANK_API_KEY')
    ...
    def set_crawler_output_path(cls, values):
        if not values.get('crawler_output_path'):
            values['crawler_output_path'] = os.path.join(values.get('default_path'), 'crawler')
        return values

    @root_validator(pre=True)
    def set_remote_url_output_path(cls, values):
        if not values.get('remote_url_output_path'):
            values['remote_url_output_path'] = os.path.join(values.get('default_path'), 'remote')
        return values
...
config = Config(open_page_rank_api_key="")
