...
def visualize_graph(graph: nx.Graph):
    ...
    nx.draw(graph, pos, with_labels=True, node_size=50, node_color="skyblue",
...
def visualize_graph_with_centrality(graph: nx.Graph):
    # Obliczanie centralności stopnia
    degree_centrality = nx.degree_centrality(graph)

    # Sortowanie węzłów według centralności stopnia malejąco
    sorted_nodes = sorted(degree_centrality.items(), key=lambda item: item[1],
                          reverse=True)

    # Przygotowanie danych do tabeli (max 10 węzłów)
    ...
    pos = nx.spring_layout(graph)
    nx.draw(graph, pos, with_labels=True, node_size=50, node_color="skyblue",
            font_size=8, font_weight="bold", edge_color="gray", ax=ax1)
    ax1.set_title("Graph of Web Pages")

    # Wizualizacja tabeli z centralnością
    ax2.axis('off')
